#!/bin/bash
# Alpine/OpenRC equivalent of assets/ec2/setup-logs.sh — installs
# /opt/app/upload-logs.sh and the logrotate stanza that calls it. Only diff
# from the AL2023 original: the upload step goes through ctech-ec2-agent's
# S3 subcommand instead of the AWS CLI (musl has no working aws-cli package).
#
# upload-logs.sh runs from logrotate's postrotate hook, so it must never fail
# the rotation: every step exits 0 on trouble.
#
# Usage: setup-logs.sh <logs-bucket> <s3-prefix> <service> <log-dir> [log-dir ...]
#   setup-logs.sh prod-ctech-application-logs ctech-billing ctech-billing /var/log/app /var/log/nginx
set -euo pipefail
...
ctech-ec2-agent s3-put -bucket __LOGS_BUCKET__ -key "__S3_PREFIX__/${DATE}-${INSTANCE_ID}.tar.gz" -file "$ARCHIVE" || exit 0
